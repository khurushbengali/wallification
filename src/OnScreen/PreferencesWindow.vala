...
using Wallification.Utilities;

namespace Wallification.OnScreen {
	...
	public class PreferencesWindow : Gtk.Window {
		...
		Button quitButton = new Button.with_label("Quit Wallification");
		...
		Gtk.CheckButton enableVideoWallpapersButton = new Gtk.CheckButton.with_label ("Enable Video Wallpapers (Restarting Wallification is required)");
		...
		public PreferencesWindow (string selectedTab = "preferences") {
			...
			// Setup Widgets
			titleLabel.set_markup("<span font='Lato Light 30px' color='white'>Wallification</span>");
			...
			quitButton.released.connect(() => {
				...
				print("Quitting Wallification. Good bye :)\n");
				Clutter.main_quit();
			...
			aboutGrid.attach(new Image.from_file("/System/Resources/Wallification/wallification.svg"), 0, 0, 1, 1);
			...
			bottomWallpapersBox.add(new Image.from_file("/System/Resources/Wallification/info.svg"));
